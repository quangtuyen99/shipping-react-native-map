            ...
            return (
                ...
                <View style={styles.container}>
                            ...
                            <View style={styles.containerItem} >
                                <TouchableOpacity
                                    ...
                                    onPress={() => {
                                        Alert.alert(
                                            'Thông báo',
                                            'Bạn có muốn xóa kiện hàng này?',
                                            [
                                                { text: 'Không', onPress: () => console.log('Cancel Press'), style: 'cancel' },
                                                {
                                                    text: 'Có', onPress: () => {
                                                        const arr = global.user.filter(item => item.id != index + 1);
                                                        this.setState({
                                                            user: arr
                                                        })
                                                        global.user = arr

                                                        for (var i = 0; i < global.user.length; i++) {
                                                            global.user[i].id = i + 1;
                                                        }




                                                        const coordinate = global.coordinates.filter(item => item.id != index + 1);
                                                        global.coordinates = coordinate;

                                                        for (var i = 0; i < global.coordinates.length; i++) {
                                                            global.coordinates[i].id = i + 1;
                                                        }



                                                        const route = global.routes.filter(item => item.id != index + 1);
                                                        global.routes = route;

                                                        for (var i = 0; i < global.routes.length; i++) {
                                                            global.routes[i].id = i + 1;
                                                        }

                                                        // global.routes.forEach((item, index) => item.id = index + 1)


                                                        // if (global.destination.length == 1) {
                                                        //     global.destination = []
                                                        // }
                                                    }
                                                },
                                ...
                                >
                                    ...
                                    <View style={styles.wrap}>
                                        ...
                                        <Text style={styles.textItem}>{item.timeStart} - {item.timeEnd} </Text>
                                    </View>
                                </TouchableOpacity>
                    ...
                    <View style={styles.buttonContainer}>
                        <TouchableOpacity
                            onPress={() => {
                                this.props.navigation.push("Ship"),
